...
def np_abs(arr):
    ...
    for i, val in enumerate(arr):
        out_arr[i] = math.abs(val)
    return out_arr

def np_where(condition, x):
    return np.array((x[condition], ))

def divide_into_frames(y, frame_size, frame_stride, fs):
    frame_len = int(frame_size) # number of samples in a single frame
    frame_step = int(frame_stride) # number of overlapping samples
    total_frames = int(np.ceil(float(abs(len(y)-frame_len))/frame_step))
    print("Frame len %d, frame step %d, total frames %d"%(frame_len, frame_step, total_frames))
    padded_y = np.concatenate((np.array(y), np.zeros(frame_len * total_frames - len(y))))
...
def absolute_threshold(new_autocorr, halflen, threshold):           
    temp = np_where(new_autocorr < threshold, new_autocorr) #np.array(np.where(new_autocorr < threshold))
    if (temp.shape == (1,0)):
        tau = -1
    else : 
        tau = temp[:,0][0]
